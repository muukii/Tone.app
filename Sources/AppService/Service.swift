...
public final class Service {
  ...
  private struct PersistentTranscriptionItem: Codable, Equatable {
    let fileName: String
    let fileURL: URL
    let tagNames: [String]
    let status: String  // "waiting", "processing", "completed", "failed"
  ...
  private struct PendingTranscriptionsWrapper: Codable, Equatable, UserDefaultsStorable {
    let items: [PersistentTranscriptionItem]
  }
    ...
    do {
      for persistedItem in persistedItems {
        ...
        let targetFile = try await actor.perform { modelContext in
          ...
          let tags = try modelContext.fetch(
            .init(
              predicate: #Predicate<TagEntity> { tag in
                tagSet.contains(tag.name)
              })
          )
